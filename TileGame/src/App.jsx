...
const App = () => {
  ...
  const loadLeaderboard = async () => {
    try {
      const response = await axios.get(`${backend}/api/leaderboard`);
  ...
  const submitScore = async (player_id, score, player_name) => {
    try {
      await axios.post(`${backend}/api/submit_score`, { player_id, score, player_name });
  ...
  const handlePlayerRegistration = async () => {
    ...
    if (id) {
      try {
        const response = await axios.get(`${backend}/api/player/${id}`);
